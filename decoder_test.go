...
var DecodableSeedCorpus = [][]byte{
	// Basic block values
	{0x20},                         // Simple B0 value (false)
	{0x21},                         // Simple B0 value (true)
	{0x10},                         // Simple B0 value (int 0)
	{0x11},                         // Simple B0 value (int 1)
	{0xFF, 0x21},                   // Level extension
	{0xFF, 0xFF, 0x21},             // Double level extension
	{0x0F, 0xAA},                   // B1 value (negative int)
	{0x0E, 0x11, 0x22},             // B2 value (negative int)
	{0x0D, 0x11, 0x22, 0x33},       // B3 value (negative int)
	{0x0C, 0x11, 0x22, 0x33, 0x44}, // B4 value (negative int)
	{0x18, 0xAA},                   // B1 value (positive int)
	{0x19, 0x11, 0x22},             // B2 value (positive int)
	{0x30, 0xAA},                   // B1 value (type 0x30)
	{0x40, 0xAA, 0xBB},             // B2 value (type 0x40)
	{0x50, 0xAA, 0xBB, 0xCC},       // B3 value (type 0x50)
	{0x70, 0x3F, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // B8 value (float 1.0)

	// String values
	{0x90, 0x00},                         // Empty string
	{0x91, 0x41, 0x42, 0x43, 0x00},       // String "ABC"
	{0x92, 0x41, 0x00, 0x01, 0x43, 0x00}, // String with escaped null byte
	{0x90, 0x41, 0x42, 0x43, 0x00},       // String with different code
	{0x91, 0x00, 0x01, 0x00, 0x01, 0x00}, // String with multiple escaped nulls

	// Tuple values
	{0xA0, 0x20},                   // T1 (boolean)
	{0xB0, 0x20, 0x21},             // T2 (two booleans)
	{0xC0, 0x20, 0x21, 0x10},       // T3 (various values)
	{0xD0, 0x20, 0x21, 0x10, 0x11}, // T4 (various values)

	// List values
	{0xF0, 0x00},                         // Empty list
	{0xF1, 0x00},                         // Empty list (different code)
	{0xF0, 0x10, 0x20, 0x30, 0xAA, 0x00}, // List with values
	{0xF0, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x00}, // List with many B0 values
...
func TestDecodableSeedCorpus(t *testing.T) {
	for i, data := range DecodableSeedCorpus {
		t.Run(fmt.Sprintf("DecodableSeedCorpus[%d]", i), func(t *testing.T) {
			_, err := Decode(data)
			if err != nil {
				t.Errorf("Failed to decode seed corpus item %d: %v", i, err)
			}
		})
	}
}
